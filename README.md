...
- **Cilium CNI support** (`cilium-setup.yml`): Installs and upgrades Cilium via `cilium-cli` (downloaded from GitHub releases — no Helm binary required). Handles idempotent install/upgrade: skips upgrade when neither the CLI version (`kubernetes_cilium_cli_version`) nor the Helm values (`kubernetes_cilium_values`) have changed. kube-proxy is skipped at `kubeadm init` time when `cni: 'cilium'` is set. `kubernetes_cilium_values.hubble.export.static` enables Hubble flow log export to `/var/run/cilium/hubble/events.log`, tailed by the OTel Collector, see `platform/otel-collector/README.md`.
